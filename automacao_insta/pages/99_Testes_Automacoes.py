"""Painel de testes e QA das automações de Instagram."""
...
from automacao_insta.config import AppConfig, load_config
from automacao_insta.db import PostStatus, get_connection, init_db
from automacao_insta.generate_calendar import generate_calendar, persist_calendar
from automacao_insta.generate_posts import generate_all_pending_posts
from automacao_insta.render_images import render_all_validated_posts, render_post_image
from automacao_insta.schedule_queue import get_posts_due, schedule_posts_for_range
from automacao_insta.validate_posts import validate_all_pending_posts, validate_post
from automacao_insta.post_instagram import publish_due_posts
...
def get_test_config() -> AppConfig:
    ...
    test_db_path = base_cfg.paths.data_dir / TEST_DB_NAME
    test_renders_dir = base_cfg.paths.renders_dir / TEST_RENDERS_DIRNAME
...
def delete_test_environment(cfg: AppConfig) -> None:
    ...
    if cfg.db.db_path.exists():
        cfg.db.db_path.unlink()
    if cfg.paths.renders_dir.exists():
        for img in cfg.paths.renders_dir.glob("*"):
            if img.is_file():
                img.unlink()
    init_db(cfg)
...
def force_validate_post(cfg: AppConfig, post: dict[str, Any]) -> None:
    ...
    from automacao_insta.db import update_post_status
...
def _render_post_timeline(post: dict[str, Any]) -> None:
    ...
    st.markdown(
        " > ".join(
            [
                f"**{s}**" if post.get("status") == s else s
                for s in statuses
            ]
        )
    )
...
def render_database_tab(cfg: AppConfig) -> None:
    ...
    status_filter = st.selectbox(
        "Filtrar por status",
        options=["todos", PostStatus.RASCUNHO, PostStatus.PARA_VALIDAR, PostStatus.VALIDADO, PostStatus.RENDERIZADO, PostStatus.AGENDADO, PostStatus.PUBLICADO, PostStatus.ERRO],
    )
