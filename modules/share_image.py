...
def _apply_noise_overlay(image: Image.Image, intensity: float = NOISE_INTENSITY) -> None:
    if intensity <= 0:
        return
    width, height = image.size
...
def _scatter_sparkles(image: Image.Image, count: int = 140) -> None:
...
def gerar_imagem_share(dados_compartilhamento: Dict[str, Any] | ShareImagePayload,
    ...
    _apply_noise_overlay(image)
    _scatter_sparkles(image)
